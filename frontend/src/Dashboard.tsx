...
function LiveFeed({ isModelsLoaded, streamError, setStreamError, selectedBatchId, isSessionActive, videoRef }: any) {
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.recognized) {
              ...
              // 8-second cooldown after a successful log to prevent false consecutive entry/exit
              if (!lastScan || Date.now() - lastScan > 8000) {
                 // Start or continue filling the timer
                 ...
                 // In cooldown - emit keep-alive so server knows they are still here
                 try {
                   socket.emit('face_detected', {
                      batchId: selectedBatchId,
                      studentId: data.student._id,
                      confidence: parseFloat(data.student.confidence)
                   });
                 } catch (err) {}
    ...
    const startLoops = () => {
      ...
      // Progress bar update interval - FILL over 3.5 seconds
      progressIntervalRef.current = setInterval(() => {
        if (recognizedStudentRef.current && lastSeenTimeRef.current) {
          const elapsed = Date.now() - lastSeenTimeRef.current;
          const progress = Math.min((elapsed / 3500) * 100, 100); // 3.5 seconds
          setDetectionProgress(progress);
          
          // Log only when fully completed (3.5 seconds continuous)
          if (progress >= 100 && (!recentlyScannedRef.current[recognizedStudentRef.current._id] || 
              (Date.now() - recentlyScannedRef.current[recognizedStudentRef.current._id] > 8000))) {
             ...
             try {
               ...
               console.log('✅ face_detected emitted for:', recognizedStudentRef.current.name, 'after 3.5s');
             } catch (err) {
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                {detectionProgress === 100 ? 'LOGGED ✓' : 'SCANNING'}
              </span>
              <span style={{
                fontSize: '14px',
                fontWeight: 800,
                ...
                {Math.max(0, (3.5 - (detectionProgress * 0.035))).toFixed(1)}s
              </span>
            </div>
          </div>
          <div style={{
